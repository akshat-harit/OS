...
{	//printf("\nEntering gt_spinlock_init in file gt_spinlock\n");
	if(!spinlock)
		return -1;
	spinlock->locked = 0;
	return 0;
...
{	//printf("\nEntering gt_actual_spinlock in file gt_spinlock\n");
	int sp_val = 1;
...
{	//printf("\nEntering gt_spin_lock in file gt_spinlock\n");
	if(!spinlock)
		return -1;
...
{	//printf("\nEntering gt_spin_unlock in file gt_spinlock\n");
	if(!spinlock)
		return -1;
