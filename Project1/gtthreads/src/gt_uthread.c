...
static int uthread_init(uthread_struct_t *u_new)
{	//printf("\nEntering uthread_init in file uthread\n");
	stack_t oldstack;
	sigset_t set, oldset;
...
{	//printf("\nEntering uthread_schedule in file uthread\n");
	...
	if((u_obj = kthread_runq->cur_uthread))
	{
		...
		if(u_obj->uthread_state & (UTHREAD_DONE | UTHREAD_CANCELLED)) //checks if thread has completed or has been cancelled
		{	//printf("If loop 1\n");
		...
		else
		{
			...
			u_obj->c_credits=u_obj->c_credits-25;
			printf("\nReducing credits to %d\n",u_obj->c_credits);
			if(u_obj->c_credits<=0){
			  u_obj->c_credits=u_obj->credits;
			  add_to_runqueue(kthread_runq->expires_runq, &(kthread_runq->kthread_runqlock), u_obj);
			  printf("Credits are zero for thread %d. Passing to expired runqueue\n", u_obj->uthread_gid);
			}
			else{
...
{	//printf("\nEntering uthread_context_func in file uthread\n");
...
extern int uthread_create(uthread_t *u_tid, int (*u_func)(void *), void *u_arg, uthread_group_t u_gid, int credits)
{	//printf("\nEntering uthread_create in file uthread\n");
	kthread_runqueue_t *kthread_runq;
