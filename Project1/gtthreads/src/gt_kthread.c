...
{	//printf("\nEntering kthread_create in file kthread\n");
	int retval = 0;
	void **stack;
	int stacksize;
...
static int kthread_handler(void *arg)
{	//printf("\nEntering kthread_handler in file kthread\n");
...
static void kthread_init(kthread_context_t *k_ctx)
{	//printf("\nEntering kthread_init in file kthread\n");
...
static inline void ksched_info_init(ksched_shared_info_t *ksched_info)
{	//printf("\nEntering ksched_info_init in file kthread\n");
...
{	//printf("\nEntering *ksched_find_target in file kthread\n");
...
{	//printf("\nEntering ksched_cosched in file kthread\n");
...
static void ksched_announce_cosched_group()
{	//printf("\nEntering ksched_announce_cosched_group in file kthread\n");
	/* Set the current running uthread_group  */
	return;
}

static void ksched_priority(int signo)
{	//printf("\nEntering ksched_priority in file kthread\n");
...
static void gtthread_app_start(void *arg)
{	//printf("\nEntering gtthread_app_start in file kthread\n");
...
extern void gtthread_app_init(char scheduler)
{	//printf("\nEntering gtthread_app_init in file kthread\n");
...

	{
		...
		int init_done;
yield_again:
		sched_yield();                      //Linux function causes the thread to yield CPU
		init_done = 0;
		for(inx=0; inx<GT_MAX_KTHREADS; inx++)  // GT_MAX_KTHREADS = GT_MAX_CORES=16
		{
			/* XXX: We can avoid the last check (tmp to cur) by
			 * temporarily marking cur as DONE. But chuck it !! */
...
{	//printf("\nEntering gtthread_app_exit in file kthread\n");
